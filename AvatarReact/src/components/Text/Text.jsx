...
export const Text = ({ plan = "Basic", setTextSpeak = () => {}, onAdd, recalculatePriceInForm }) => {
  const [count, setCount] = useState(0);
  const recalculate = (e) => {
    let wordsCollection = e.target.value.trim().split(/\s+/g)
    setCount(wordsCollection.length);
    setTextSpeak(e.target.value);
    onAdd(wordsCollection.length, "text");
    onAdd(wordsCollection.length, "numberWords");
    recalculatePriceInForm(plan, wordsCollection.length);
  };
    ...
    <>
      <div className="limit-plan flex w-full items-center justify-end gap-1.5">
        ...
        <div className="plan flex items-center gap-1">
          <span className="text-black text-2xl">{t("basicPlan")}</span>
          <svg
            width={17}
            height={16}
            viewBox="0 0 17 16"
